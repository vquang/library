...
public class ThongKeDAO {
    ...
    public List<ThongKeBanDoc> getListTKBanDocs(String thuTu) {
        ...
        try {
            DBConnect dbConnect = new DBConnect();
            Connection connection = dbConnect.getConnection();
            String sql = "SELECT tv.id, tv.hoTen, tv.soDienThoai, tv.diaChi, tv.email, tv.vaiTro, COUNT(tm.id) AS tongTaiLieu "
                    + "FROM ThanhVien tv "
                    + "JOIN PhieuMuon pm ON tv.id = pm.banDocId "
                    + "LEFT JOIN TaiLieuMuon tm ON pm.id = tm.phieuMuonId "
                    + "GROUP BY tv.id, tv.hoTen, tv.soDienThoai, tv.diaChi, tv.email, tv.vaiTro "
